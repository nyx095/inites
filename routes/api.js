...
let Creator = ["NYXChan"]
let notesuccess = ["Jangan Di Tembak Bang Sakit:)"]
let creator = Creator[Math.floor(Math.random() * Creator.length)];
let notes = notesuccess[Math.floor(Math.random() * notesuccess.length)];
let listkey = ["NYX-Chan"];
let ownerNum = "6281216465289"
...
loghandler = {
    ...
    apikey: {
         ...
         creator: `${creator}`,
         code: 404,
         message: 'apikey belum di input, atau apikey salah'
     },
    error: {
        status: 404,
        creator: `${creator}`,
        message: `An internal error occurred. Please report via WhatsApp wa.me/${ownerNum}`
    }
}
